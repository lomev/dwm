...
void
deck(Monitor *m) {
	unsigned int i, n, h, mw, my, ns, bw, gap;
	Client *c;
	...
	for(i = 0, my = gap, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if(i < m->nmaster) {
			...
			resize(c, m->wx + gap, m->wy + my, mw - (2*bw) - gap*(5-ns)/2, h - (2*bw), bw, 0);
			my += HEIGHT(c) + gap;
		}
		else
			resize(c, m->wx + mw + gap/ns, m->wy + gap, m->ww - mw - (2*bw) - gap*(5-ns)/2, m->wh - (2*bw) - 2*gap, bw, 0);
}
...
updatemotifhints(Client *c)
{
	...
	if (XGetWindowProperty(dpy, c->win, motifatom, 0L, 5L, False, motifatom,
		...
		if (motif[MWM_HINTS_FLAGS_FIELD] & MWM_HINTS_DECORATIONS) {
			...
			resize(c, c->x, c->y, width - (2*bw), height - (2*bw), bw, 0);
		}
		XFree(p);
	}
}
...
updatesystrayicongeom(Client *i, int w, int h)
{
	...
	if (i) {
		...
		applysizehints(i, &(i->x), &(i->y), &(i->w), &(i->h), bw, 0);
